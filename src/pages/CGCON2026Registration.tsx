...
const CGCON2026Registration = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const pkgLabel = pkg ? `${pkg.description} ${pkg.price}` : selectedPkg;
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8" noValidate>
                        ...
                        <div className={`pt-4 p-5 rounded-lg ${errors.selectedPkg ? 'bg-red-50 border border-red-200' : 'bg-transparent'}`}>
                            ...
                            <div className="max-w-3xl mx-auto space-y-2">
                                {PACKAGES.map(pkg => (
                                    <label key={pkg.id} className="grid grid-cols-[auto_1fr_auto] items-center gap-x-3 text-sm font-semibold text-gray-800 cursor-pointer p-2 hover:bg-gray-50 rounded transition">
                                        <input type="radio"
                                        ...
                                        <span className="tabular-nums text-right whitespace-nowrap min-w-[5.5rem] sm:min-w-[6rem]">{pkg.price}</span>
                                    </label>
                                ))}
                            </div>
